...
def validate_custom_doc(file_path: str) -> ValidationResult:
    ...
    if content.strip().startswith("# "):
        result.errors.append(
            "File starts with class title (# ClassName) - this is old format"
        )
        result.suggestions.append(
            "Remove the class title line - only include section headers (##)"
        )

    if re.search(r"^## Declaration", content, re.MULTILINE):
        result.errors.append(
            "Contains ## Declaration section - this is structural duplication"
        )
    ...
    has_any_content = bool(
        re.search(
            r"^## (Overview|Field Descriptions|Constructor Descriptions|Method Descriptions|Usage Notes|Examples)",
            content,
            re.MULTILINE,
        )
    )
    ...
    if not has_any_content:
        result.errors.append(
            "No valid sections found - file appears to be in old format or empty"
        )
        result.suggestions.append(
            "Use new format with ## Overview, ## Method Descriptions, etc."
        )
    ...
    for pattern, msg in generic_patterns:
        if re.search(pattern, content):
            ...
            result.suggestions.append(
                "Replace generic descriptions with specific, useful information"
            )

    # Check method description format
    method_section = re.search(
        r"## Method Descriptions\s*\n(.*?)(?=\n##|\Z)", content, re.DOTALL
    )
    if method_section:
        ...
        if not re.search(r"-\s+`\w+\([^)]*\)`:", method_content):
            result.warnings.append(
                "Method descriptions don't follow format: - `methodName()`: description"
            )

    # Check field description format
    field_section = re.search(
        r"## Field Descriptions\s*\n(.*?)(?=\n##|\Z)", content, re.DOTALL
    )
    if field_section:
        field_content = field_section.group(1)
        if not re.search(r"-\s+`[A-Za-z_][A-Za-z0-9_]*`:", field_content):
            result.warnings.append(
                "Field descriptions don't follow format: - `FIELD_NAME`: description"
            )
    ...
    if has_overview:
        ...
        if overview_match:
            ...
            if len(overview_text) < 20:
                result.warnings.append(
                    "Overview is very short - consider adding more context"
                )
            elif overview_text.startswith("-"):
                result.warnings.append(
                    "Overview uses bullet points - prefer prose description"
                )
...
def main():
    ...
    parser.add_argument(
        "--docs", "-d", default="docs", help="Custom docs directory (default: docs)"
    )
    parser.add_argument("--file", "-f", help="Validate a single file")
    parser.add_argument(
        "--quiet", "-q", action="store_true", help="Only show files with issues"
    )
